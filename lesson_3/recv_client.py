...
@log
def get_message_from_chat(message):
    if ACTION in message and message[ACTION] == MESSAGE and MESSAGE_TEXT in message:
        return f'Сообщение от {message[SENDER]} \n' \
...
def main():
    ...
    try:
        ...
        print('Вы подключились к чату. Роль - Получатель')
    except (ValueError, json.JSONDecodeError):
        LOG.error('Failed to decode server message.')

    while True:
        msg_from_chat = get_message_from_chat(get_message(transport))
        print(msg_from_chat)
