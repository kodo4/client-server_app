...
@log
def create_message(msg, account_name='Guest'):
    out = {
        ACTION: MESSAGE,
        TIME: time.time(),
        USER: {
    ...
    LOG.info(f'Сообщение в общий чат: ', out)
    return out
...
def main():
    ...
    try:
        ...
        print('Вы подключились к чату. Роль - Отправитель')
    ...
    while True:
        ...
        if msg == 'exit()':
            break
        try:
            message_to_server = create_message(msg)
            send_message(transport, message_to_server)
            LOG.info(f'Сообщение {msg} отправлено на сервер')
        except:
            LOG.critical('Что-то пошло не так.')
